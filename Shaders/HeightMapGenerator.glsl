...
void main()
{
    ...
    uint x = id % parameters.size;
    uint y = id / parameters.size;

    float scale = parameters.scale;
    float weight = 1;
    for (int i = 0; i < parameters.octaves; i ++) {
        heightMap[id] += cnoise(vec2(x,y) / parameters.size * parameters.scale + parameters.octaveoffsets[i]) * weight;
        scale *= parameters.lacunarity;
