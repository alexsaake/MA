...
void Cascade(ivec2 position)
{
    ...
    for (int neighbor = 0; neighbor < neighbors; neighbor++)
    {
        ...
        float excess = abs(heightDifference) - neighborCells[neighbor].distance * thermalErosionConfiguration.TangensTalusAngle / mapGenerationConfiguration.HeightMultiplier;
        if (excess <= 0)
        {
            continue;
        }
