...
void RemoveFrom(uint index, uint layer, float sediment)
{
    uint offsetIndex = index + layer * myHeightMapLength;
    heightMap[offsetIndex] = max(heightMap[offsetIndex] - sediment, 0.0);
}

void DepositeOn(uint index, uint layer, float sediment)
{
    heightMap[index + layer * myHeightMapLength] += sediment;
}
...
{    
    ...
    for(uint layer = 0; layer < mapGenerationConfiguration.LayerCount; layer++)
    {
        ...
        if(volumeDelta < 0)
        {
            RemoveFrom(index, layer, abs(volumeDelta));
        }
        else
        {
            if(layer == mapGenerationConfiguration.LayerCount - 1)
            {
                DepositeOn(index, layer, volumeDelta);                
            }
            else
            {
                DepositeOn(index, layer + 1, volumeDelta);
            }
        }
    }
